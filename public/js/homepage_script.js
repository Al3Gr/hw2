fetch("/meteo").then(onResponse).then(onJsonWeather)
...
for(let i of delete_buttons){
    i.addEventListener("click", closePostForm)
}
...
function closePostForm(event){
    ...
    for(let i of overlays){
        const title = i.querySelector(".title_post")
        const textarea = i.querySelector(".comment_post")
        title.value=""
        textarea.value=""
...
function deletePost(event){
    ...
    fetch("/home/deletePost/"+encodeURIComponent(post.dataset.id))
}
...
function createCommentSection(post_id){
    ...
    insert_comment.appendChild(csrf_input)
